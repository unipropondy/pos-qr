    ...
    const receiptCount = Array.isArray(items) ? items.filter(i => i.status !== "VOIDED").reduce((sum, item) => sum + (Number(item.qty) || 0), 0) : 0;
      ...
      if (tableId) {
        try {
          ...
          const tableRes = await transaction.request()
            .input("tid", sql.NVarChar(128), cleanTableId)
            .query("SELECT entry_status FROM TableMaster WHERE TableId = @tid");
          
          if (tableRes.recordset[0]?.entry_status === 'q') {
            isQROrder = true;
          }
        } catch (qrErr) {
          console.warn("⚠️ [QR Check in Sales] Error checking table entry status:", qrErr.message);
        }
      }

      // 🚀 PROFESSIONAL ARCHIVE: Move from Cur to History (Only run if not split, or if split has no remaining items)
      // For QR orders, do NOT archive at payment step. It will stay in RestaurantOrderCur until served.
      if (displayOrderId && (!isSplit || !hasRemaining) && !isQROrder) {
        try {
          await transaction.request()
      ...
      if (tableId) {
        ...
        if (isSplit && hasRemaining) {
          ...
          // 🌟 QR ORDER: Activate Table (Status = 1) instead of clearing. Do NOT clear CartItems or closed order status.
          console.log(`[SAVE SALE] [QR Order Flow] Activating table and setting PAYMENT_STATUS = 1: ${cleanTableId}`);
          await transaction.request()
            .input("tid", sql.NVarChar(128), cleanTableId)
            .query("UPDATE [dbo].[TableMaster] SET Status = 1, PAYMENT_STATUS = 1 WHERE TableId = @tid");

          const io = req.app.get("io");
          if (io) {
            io.emit("table_status_updated", { tableId: cleanTableId.toLowerCase(), status: 1, paymentStatus: 1 });
            io.emit("tables_updated");
            // Since table went to Status 1, release items to KDS immediately by sending cart_updated
            io.emit("cart_updated", { tableId: cleanTableId.toLowerCase(), orderId: displayOrderId });
          }
        } else {
          ...
          // 🚀 GLOBAL KDS SYNC: Mark order as closed in professional tables (skip for QR orders so KDS can display it)
          if (!isQROrder) {
            await transaction.request()
              .input("orderNo", sql.NVarChar(50), displayOrderId)
