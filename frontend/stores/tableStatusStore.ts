...
export const useTableStatusStore = create<TableStatusState>((set, get) => ({
  ...
  updateTableStatus: (tableId, section, tableNo, orderId, status, startTime, lockedByName, totalAmount, isExternal, isHoldOvertime, modifiedOn, entryStatus, paymentStatus) => {
    ...
    set((state) => {
      ...
      const updatedTable: TableStatus = {
        ...
        paymentStatus: paymentStatus !== undefined ? paymentStatus : (existingIndex > -1 ? state.tables[existingIndex].paymentStatus : undefined),
      };
...
export const updateTableStatus = (
  ...
  entryStatus?: string,
  paymentStatus?: number
) => useTableStatusStore.getState().updateTableStatus(tableId, section, tableNo, orderId, status, startTime, lockedByName, totalAmount, isExternal, isHoldOvertime, modifiedOn, entryStatus, paymentStatus);
